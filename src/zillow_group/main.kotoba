(ns zillow-group.main
  "Kotodama WASM entrypoint for the Zillow_group clean-room actor (L4) — Clojure port.
  ...
  facts (`zillow_group.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "zillow_group")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"     :plural "listings"     :id-prefix "zillowgr_lis"
   ...
   {:entity "Tenant"      :plural "tenants"       :id-prefix "zillowgr_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"zillow_group.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zillow_group-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zillow_group-compat/ready)
